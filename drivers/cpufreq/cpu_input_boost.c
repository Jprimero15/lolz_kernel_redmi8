...
static unsigned int cpu_freq_default_min_lp __read_mostly =
	CONFIG_CPU_FREQ_DEFAULT_MIN_LP;
static unsigned int cpu_freq_default_min_hp __read_mostly =
	CONFIG_CPU_FREQ_DEFAULT_MIN_HP;
...
module_param(cpu_freq_default_min_lp, uint, 0644);
module_param(cpu_freq_default_min_hp, uint, 0644);
...
static unsigned int get_min_freq(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(policy->cpu, cpu_lp_mask))
		freq = cpu_freq_default_min_lp;
	else
		freq = cpu_freq_default_min_hp;
